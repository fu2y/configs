...
local wp_path = os.getenv("HOME").."/documents/wallpaper"
...
-- /home fs
-- shows warning if hdd is filled with more than 80%
fshome = lain.widgets.fs({
    ...
    settings  = function()
        ...
        if fs_now.used >= 80 then
            fs_header = " DISK SPACE USAGE: "
            fs_header2 = " % "
            fs_p      = fs_now.used
        end

        widget:set_markup(markup(red, fs_header) .. fs_p .. fs_header2)
    end
})
